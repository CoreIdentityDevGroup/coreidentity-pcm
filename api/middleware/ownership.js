...
// Staff roles (facilitator, program_manager, intake_officer) keep their
// existing, unrestricted scope over client-linked resources -- this only adds
// a floor under the 'client' role. One middleware (fetch the resource,
// compare client_id, 403 on mismatch) instead of copy-pasting that check
// per route.
//
...
// asset_id), pass an async function that does that lookup and returns the
// client_id, or null if the resource doesn't exist -- in the null case this
// middleware lets the request through so the route's own 404 handling runs
// normally instead of this middleware masking it as a 403.
