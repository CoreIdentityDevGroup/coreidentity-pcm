...
router.post('/:type', authorize('facilitator'), async (req, res, next) => {
  try {
...
router.put('/:type/:id', authorize('facilitator'), async (req, res, next) => {
  try {
    const meta = resolveType(req, res);
    if (!meta) return;
...
router.delete('/:type/:id', authorize('facilitator'), async (req, res, next) => {
  try {
    const meta = resolveType(req, res);
    if (!meta) return;
